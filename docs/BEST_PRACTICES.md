...
This guide provides battle-tested advice for implementing data validations in production environments. Learn when to use ERROR vs WARNING, what minimum checks every dataset needs, and how to handle failures pragmatically.
...
**Rationale**: Fail fast on simple checks before spending time on complex validations.
...
# Check exit code
if [ $? -ne 0 ]; then
  echo "CRITICAL: Validation failed with errors"
...
if [ "$WARNING_COUNT" -gt 0 ]; then
  echo "WARNING: Found $WARNING_COUNT validation warnings"
...
while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
  ...
  echo "Validation failed, attempt $RETRY_COUNT of $MAX_RETRIES"
...
- [ ] **Run profiler** on production-like data
  ```bash
...
6. **Test Before Production**: Use profiler and historical data
...
- **[Validation Catalog](VALIDATION_CATALOG.md)** - Complete list of available validations
- **[Examples & Recipes](EXAMPLES_AND_RECIPES.md)** - More real-world scenarios
- **[Advanced Configuration](ADVANCED_CONFIGURATION.md)** - Complex validation patterns
- **[User Guide](USER_GUIDE.md)** - Complete configuration reference

---

**Questions or suggestions?** [Open an issue](https://github.com/danieledge/data-validation-tool/issues) or contribute to this guide!
