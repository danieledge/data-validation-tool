...
**Complete guide to using the Data Validation Framework**

This guide is for business analysts, data engineers, and anyone who needs to validate data quality without writing code.
...
The Data Validation Framework helps you ensure data quality by checking your data against rules you define. Everything is configured in YAML - no programming required!

**Key Features:**
- ✅ **No coding required** - Define validations in simple YAML configuration
- 📊 **Multiple file formats** - CSV, Excel, JSON, Parquet
- 🔄 **Handles large files** - Processes data in chunks for memory efficiency
...
**Tips:**
- Parquet is very fast for large files (GB+)
...
See the **[Validation Catalog](VALIDATION_CATALOG.md)** for complete list with examples.
...
**Impact:** If any ERROR validations fail, the overall validation status is FAILED.
...
```yaml
settings:
  chunk_size: 5000  # Process more rows at once
...
```yaml
settings:
  chunk_size: 10000  # Process 10,000 rows at once
```
...
3. **Check validation catalog** - See [Validation Catalog](VALIDATION_CATALOG.md)
4. **Run with --verbose** - Get detailed output
5. **Check logs** - Look in `logs/` directory
6. **Review technical docs** - See [Technical Guide](TECHNICAL_GUIDE.md)

---

## Next Steps

- **[Validation Catalog](VALIDATION_CATALOG.md)** - Complete reference of all validation types
...
- **[Technical Architecture](TECHNICAL_ARCHITECTURE.md)** - How the system works internally

---

**Need more help?** Check out the other documentation or review the examples in the repository.
